...
class TwitterScraper(BaseScraper):
    ...
    def get_actor_id(self): return ACTOR_IDS["x"]

    def build_input(self, username, date_from, date_to):
        clean = username.lstrip("@").strip()
        return {
            "startUrls": [f"https://twitter.com/{clean}"],
    ...
    def parse_results(self, raw_items, date_from, date_to):
        ...
        for item in raw_items:
            ...
            likes = self.safe_int(item.get("likeCount") or item.get("likes")
                                  or item.get("favorite_count") or item.get("favoriteCount"))
            comments = self.safe_int(item.get("replyCount") or item.get("replies")
            ...
            has_video = item.get("video") or item.get("isVideo") or any(
            ...
            url = item.get("url") or item.get("tweetUrl") or item.get("twitterUrl") or ""
