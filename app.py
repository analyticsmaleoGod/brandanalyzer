...
st.set_page_config(page_title="Brand Analyzer", page_icon="📊", layout="wide", initial_sidebar_state="collapsed")
...
def login_page():
    st.markdown("""<style>.block-container{max-width:440px;padding-top:10vh}</style>""", unsafe_allow_html=True)
...
api_token = get_token("APIFY_API_TOKEN")
claude_key = get_token("ANTHROPIC_API_KEY")
...
T_CP, T_FG, T_CMT, T_KW, T_HT = st.tabs([
    ...
    "🔍 Keyword Tracker", "#️⃣ Hashtag Tracker",
])
...
def plat_grid(prefix, default_on=True):
    ...
    for i, k in enumerate(pkeys):
        ...
        with (r1 + r2)[i]:
            ...
            if on and u.strip(): sel[k] = u.strip()
    return sel


def date_pick(prefix):
    a, b = st.columns(2)
    with a: df = st.date_input("From", value=datetime.now()-timedelta(days=90), max_value=datetime.now(), key=f"{prefix}_f")
    with b: dt = st.date_input("To", value=datetime.now(), max_value=datetime.now(), key=f"{prefix}_t")
    return df, dt
...
with T_CP:
    ...
    if not api_token: st.warning("Apify token not configured. Contact admin.")
    if st.button(f"🔍 Pull Data — {n} platform{'s' if n!=1 else ''}", type="primary",
        ...
        for i, (pk, uname) in enumerate(sel.items()):
            ...
            sc = SCRAPERS.get(pk)
            if not sc: errs.append(f"{pf['label']}: not found"); continue
            try:
                r = sc(api_token).scrape(uname, datetime(d_from.year,d_from.month,d_from.day),
                    datetime(d_to.year,d_to.month,d_to.day,23,59,59))
                posts.extend(r)
                status.markdown(f"✅ **{pf['label']}** — {len(r)} posts")
            except Exception as e: errs.append(f"{pf['label']}: {e}")
...
with T_CMT:
    ...
    st.caption("One URL per line — mix Instagram & TikTok links. System auto-detects platform.")
    ...
    if st.button(f"💬 Pull Comments — {len(parsed)} posts", type="primary", use_container_width=True, disabled=not can_cmt, key="cmt_go"):
        status = st.empty()
    ...
    if st.session_state.get("cmt_r"):
        ...
        if cmt:
            ...
            if "comment" not in cdf.columns: cdf["comment"] = ""
            ...
            n_src = cdf["source"].nunique()
            st.success(f"✅ **{len(cdf)} comments** from {n_src} posts")
            c1,c2,c3,c4 = st.columns(4)
            with c1: st.metric("Comments", f"{len(cdf):,}")
            with c2: st.metric("Posts scraped", n_src)
            ...
            c1,c2,_ = st.columns([1,1,2])
            with c1: st.download_button("📥 .xlsx", export_comments_to_excel(cmt), "comments.xlsx", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", use_container_width=True, key="cmt_dx")
            with c2: st.download_button("📥 .csv", cdf.to_csv(index=False), "comments.csv", "text/csv", use_container_width=True, key="cmt_dc")
        else: st.markdown("---"); st.warning("No comments found.")
...
def search_tab(tab_key, search_type, label, placeholder, icon):
    ...
    raw = st.text_input(f"{label}s", placeholder=placeholder, key=f"{tab_key}_input", help=f"Separate with commas")
    queries = [q.strip() for q in raw.split(",") if q.strip()] if raw else []
    if queries: st.caption(f"{len(queries)} {label.lower()}{'s' if len(queries)>1 else ''}: " + ", ".join([f"`{q}`" for q in queries]))
    ...
    for i, k in enumerate(sp_keys):
        ...
        with plat_cols[i]:
            if st.checkbox(f"{sp['icon']} {sp['label']}", value=i<2, key=f"{tab_key}_p_{k}"): sel_plats.append(k)
    st.markdown("### Result limit")
    mode = st.radio("Mode", ["By quantity (per platform)", "By date range"], horizontal=True, key=f"{tab_key}_mode", label_visibility="collapsed")
    max_qty, dt_from, dt_to = 100, None, None
    if "quantity" in mode:
        max_qty = st.slider("Max results per platform", 10, 1000, 100, 10, key=f"{tab_key}_qty"); mode_key = "quantity"
    else:
        c1, c2 = st.columns(2)
        ...
        mode_key = "daterange"
        days_back = max((datetime.now().date()-dt_from).days, 1)
        max_qty = max(30, min(int(days_back*2*1.3), 1000))
    st.markdown("")
    can = bool(api_token and queries and sel_plats)
    if st.button(f"{icon} Search — {len(queries)} {label.lower()}{'s' if len(queries)>1 else ''} × {len(sel_plats)} platform{'s' if len(sel_plats)>1 else ''}", type="primary", use_container_width=True, disabled=not can, key=f"{tab_key}_go"):
        status = st.empty()
        def search_cb(msg): status.markdown(msg)
        results = search_multi(api_token, queries, sel_plats, search_type, mode_key, max_qty,
    ...
    if st.session_state.get(f"{tab_key}_r"):
        ...
        if results:
            ...
            st.dataframe(disp.sort_values("Likes",ascending=False), use_container_width=True, height=400, hide_index=True)
            c1,c2,_ = st.columns([1,1,2])
            with c1: st.download_button("📥 .xlsx", export_search_to_excel(results, queries_used, search_type), f"{search_type}_search.xlsx", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", use_container_width=True, key=f"{tab_key}_dx")
            with c2: st.download_button("📥 .csv", rdf.to_csv(index=False), f"{search_type}_search.csv", "text/csv", use_container_width=True, key=f"{tab_key}_dc")
        else: st.markdown("---"); st.warning(f"No results found.")

with T_KW: search_tab("kw", "keyword", "Keyword", "e.g. skincare routine, glass skin, morning routine", "🔍")
with T_HT: search_tab("ht", "hashtag", "Hashtag", "e.g. #CleanBeauty, #SkincareRoutine, #GlassSkin", "#️⃣")
