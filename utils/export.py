...
def _build_summary(df, platforms_used, date_from, date_to):
    ...
    for p in platforms_used:
        ...
        if not pdf["type"].empty:
            ...
            rows.append({"Section": "", "Metric": "Top content type", "Value": f"{top.index[0]} ({round(top.iloc[0]/pc*100,1)}%)"})
        rows.append({"Section": "", "Metric": "", "Value": ""})
...
def export_to_excel(all_posts, platforms_used, date_from, date_to, ai_analysis=""):
    ...
    if df.empty:
        df = pd.DataFrame(columns=["platform","date","type","caption","likes","comments","shares","saves","views","engagement_rate","url"])
    ...
    posts_df = posts_df.rename(columns={
        "platform": "Platform", "date": "Date", "type": "Type", "caption": "Caption / Title",
        "likes": "Likes", "comments": "Comments", "shares": "Shares", "saves": "Saves",
        "views": "Views", "engagement_rate": "ER (%)", "url": "URL",
    })
...
def export_to_csv(all_posts):
    ...
    if df.empty:
        return ""
    return df.sort_values("date", ascending=False).to_csv(index=False) if not df.empty else df.to_csv(index=False)
...
def export_comments_to_excel(comments):
    ...
    all_df = df.sort_values("likes", ascending=False).rename(columns={
        ...
        "comment": "Comment", "likes": "Likes", "date": "Date", "is_reply": "Is Reply",
        "source_url": "Source URL",
    })

    # Sheet 2: Per post breakdown
...
def export_search_to_excel(posts, queries, search_type="keyword"):
    ...
    for q in queries:
        ...
        summary_rows.append({"Section": "", "Metric": "Top platform", "Value": qdf["platform"].value_counts().index[0] if len(qdf) else "—"})
        summary_rows.append({"Section": "", "Metric": "", "Value": ""})
    ...
    # Sheet 2: All posts
    all_df = df.sort_values("likes", ascending=False).rename(columns={
        "query": label, "platform": "Platform", "username": "Username", "date": "Date",
        "caption": "Caption", "likes": "Likes", "comments": "Comments", "shares": "Shares",
        "views": "Views", "engagement_rate": "ER (%)", "url": "URL",
    })
